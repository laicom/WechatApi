 ...
 * Update History:  
 *      2015-01-28  Add Sha1 Hash method
  */
...
namespace Nlab.WeChatApi.Utilities
{
    class Cryptography
    {
        ...
        {
            var inputBuf = System.Text.Encoding.UTF8.GetBytes(s);
            var sha = SHA1.Create("sha1");
            var outputBuf = sha.ComputeHash(inputBuf);
            var result = outputBuf.BytesToHexString().ToLower();
            return result;
        }
